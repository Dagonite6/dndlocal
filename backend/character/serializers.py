...
def race_validator(race):  
    if not race in constants.RACES:
        raise serializers.ValidationError({"race": "Unknown or empty race"})
    
def class_validator(char_class):
    if not char_class in constants.CLASSES:
        raise serializers.ValidationError({"race": "Unknown or empty class"})
